...
export function localizedDropToInternal(
  fromPlatform: string,
  from: LocalizedDrop,
  identifierToInternal: (model: string, localId: string) => string,
): InternalDrop {
  ...
  const ret: InternalDrop = {
    tubsId:
      from.foreignIds['tubs'] || identifierToInternal(from.model, from.localId),
    platformIds: {},
    properties: {},
    relations: {},
    model: from.model,
...
export function internalDropToLocalized(
  toPlatform: string,
  from: InternalDrop,
  identifierToLocal: (model: string, tubsId: string) => string,
): LocalizedDrop {
  ...
  Object.keys(from.relations).forEach((relation) => {
    ret[`${relation}Id`] = identifierToLocal(
      relation,
      from.relations[relation].tubsId,
    );
  });
  return ret;
}
