...
function getIssueState(entry: object): string | undefined {
  ...
  if ((typeof typeStr === 'string') && (typeStr.startsWith('http://www.w3.org/2005/01/wf/flow#'))) {
    return typeStr.substring('http://www.w3.org/2005/01/wf/flow#'.length);
  }
  return undefined;
}

export function interpret({ index, state }: { index: TrackerIndex, state: TrackerState }): Interpretation {
  const ret = {
    tracker: {
  ...
  state.forEach((entry: TrackerStateEntry) => {
    ...
    if (typeof issueState === 'string') {
      ...
      const commentLinksForIssue = getJsonLdStringFieldMulti(entry, 'http://www.w3.org/2005/01/wf/flow#message');
  ...
  commentLinks.forEach(({ issue, comment }: {
    issue: string,
    comment: string,
  }) => {
